...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) => subcommand
            ...
            .addStringOption((option) =>
                option
                .setName("attribute")
                .setDescription("The prestige attribute you want to increase")
                .setRequired(true)
                .addChoices([
                    { name: "Experience Boost", value: "ExperienceBoost" },
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (subcommand === "show") return interaction.reply({ embeds: [embed] })
            ...
            if(attr.value >= 10 || attr.value + amount > 10) return interaction.reply({ content: `You can't increase an attribute past 10!` });
            if(player.getStatPoints() < amount) return interaction.reply({ content: `You can't increase an attribute with more stat points than you have!` });
            if(!player.getAttributes().find(atr => atr.name === attribute)) return interaction.reply({ content: `${attribute} is not a valid attribute!`});
            ...
            const attribute = options.getString("attribute", true)
            if(player.getPrestigePoints() <= 0) return interaction.reply({ content: `You need at least 1 prestige point to increase a prestige attribute` });
            if(!player.getPrestigeAttributes().find(atr => atr.name === attribute)) return interaction.reply({ content: `${attribute} is not a valid attribute!`});
            player.increasePrestigeAttribute(attribute).save();
            return interaction.reply({ content: `Your ${player.getPrestigeAttribute(attribute).name} is now ${player.getPrestigeAttribute(attribute).value}!` });
